...
def setup(ctx: PluginContext):
    ...
    async def on_forward(app, event):
        ...
        if int(getattr(event, "chat_id", 0) or 0) != app.self_id: return
        fwd = event.forward
        if fwd is None: return
        text = (event.raw_text or "").strip()
        if text.startswith(app.config.cmd_prefix): return

        source_id = None
        if fwd.chat_id:
            source_id = int(fwd.chat_id)
        elif fwd.from_id:
            try:
                from telethon import utils as tu
                source_id = int(tu.get_peer_id(fwd.from_id, add_mark=True))
        ...
        try:
            ...
            if getattr(entity, "megagroup", False) or getattr(entity, "gigagroup", False): stype = "超级群"
            elif getattr(entity, "broadcast", False): stype = "频道"
            elif hasattr(entity, "participants_count"): stype = "群组"
            else: stype = "私聊"
        except Exception:
            title, stype = "无法获取", "未知"
        ...
        if tips: secs.append(ui.section("快捷操作", tips))
