...
def _safe_shlex(args: str) -> list[str]:
    """FIX BUG-04: shlex.split with fallback on unclosed quotes."""
    try:
...
async def cmd_ping(app, event, args):
    ...
    await app.safe_reply(
        ...
        panel(
            "TR 管理器 · 在线心跳",
            [section("快速状态", [
                bullet("管理层运行", format_duration((datetime.now() - app.started_at).total_seconds())),
                bullet("历史命中", stats.get("total_hits", "0")),
                bullet("热更新", "事件驱动"),
                bullet("自动同步", f"{app.config.auto_sync_time} 每日执行" if app.config.auto_sync_enabled else "已关闭", code=False),
            ])],
        ),
        auto_delete=12,
        prefer_edit=False,
        ...
        panel(
            ...
            [
                section("当前构建", [
                    ...
                    bullet("架构", "插件全解耦 / Admin + Core / SQLite WAL"),
                    bullet("终端管理器", "TR"),
                ]),
                section("部署位置", [
                    bullet("工作目录", shorten_path(app.config.work_dir), code=False),
                    bullet("交互策略", "轻命令直回 / 重命令排队", code=False),
                ]),
            ],
        ),
        prefer_edit=False,
...
async def cmd_log(app, event, args):
    ...
    for row in rows:
        ...
        blocks.append("\n".join(lines))
    footer = "<i>默认只展示关键事件。发送 <code>{0}log normal 20</code> 可看更多，<code>{0}log all 20</code> 完整事件流。</i>".format(app.config.cmd_prefix)
