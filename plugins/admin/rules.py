...
def _safe_split(args: str) -> list[str]:
    try:
...
async def cmd_addrule(app, event, args):
    tokens = _safe_split(args)
    if len(tokens) < 3:
        await app.safe_reply(event, panel("TR 管理器 · 参数不足", [section("示例", [f"<code>{app.config.cmd_prefix}addrule 示例分组 规则A 监控词A 监控词B</code>", f"<code>{app.config.cmd_prefix}setrule 示例分组 规则A 全量表达式</code>"])]), prefer_edit=False)
        return
...
async def cmd_setprefix(app, event, args):
    value = args.strip()
    if not value or len(value) > 3 or not re.match(r'^[^\s\\\'\"<>&]+$', value):
        await app.safe_reply(event, panel("TR 管理器 · 命令前缀格式无效", [section("输入要求", [bullet("长度", "1-3 个字符", code=False), bullet("限制", "不能包含空格、引号、反斜杠、HTML 字符", code=False)])]), prefer_edit=False)
        return
